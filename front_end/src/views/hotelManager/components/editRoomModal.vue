<template xmlns:a-radio="http://www.w3.org/1999/html">
    <a-modal
            :visible="editRoomModalVisible"
            :width=620
            title="更新客房信息"
            cancelText="取消"
            okText="确定"
            @cancel="cancel"
            @ok="handleSubmit"
    >
        <a-form :form="form" style="margin-top: 30px" v-bind="formItemLayout">
            <a-form-item label="房型" v-bind="formItemLayout">
                <a-select
                        v-decorator="[
                    'roomType',
                    { rules: [{ required: true, message: '请选择房型' }] }]"
                >
                    <a-select-option value="BigBed">大床房</a-select-option>
                    <a-select-option value="DoubleBed">双床房</a-select-option>
                    <a-select-option value="Family">家庭房</a-select-option>
                </a-select>
            </a-form-item>
            <a-form-item label="房间数量" v-bind="formItemLayout">
                <a-input
                        placeholder="请填写房间数量"
                        v-decorator="['roomNum', { rules: [{ required: true, message: '请输入房间数量' }] }]"
                />
            </a-form-item>
            <a-form-item label="原始价格" v-bind="formItemLayout">
                <a-input
                        placeholder="请填写原始价格"
                        v-decorator="['price', { rules: [{ required: true, message: '请输入原始价格' }] }]"
                />
            </a-form-item>
            <a-form-item label="面积" v-bind="formItemLayout">
                <a-input
                        placeholder="请填写面积"
                        v-decorator="['area', { rules: [{ required: false}] }]"
                />
            </a-form-item>
            <a-form-item label="楼层" v-bind="formItemLayout">
                <a-input
                        placeholder="请填写楼层"
                        v-decorator="['level', { rules: [{ required: false}] }]"
                />
            </a-form-item>
            <a-form-item label="床型" v-bind="formItemLayout">
                <a-input
                        placeholder="请填写床型"
                        v-decorator="['bedType', { rules: [{ required: false}] }]"
                />
            </a-form-item>
            <a-form-item label="可否加床" v-bind="formItemLayout">
                <a-radio-group name="addBed" :default-value=false
                               v-decorator="['addBed', { initialValue:false} ]">
                    <a-radio :value=true>
                        可
                    </a-radio>
                    <a-radio :value=false>
                        否
                    </a-radio>
                </a-radio-group>
            </a-form-item>
            <a-form-item label="设施" v-bind="formItemLayout">
                <a-textarea
                        placeholder="请填写设施"
                        rows="7"
                        v-decorator="['facility', { rules: [{ required: false}] }]"
                />
            </a-form-item>
        </a-form>
    </a-modal>
</template>
<script>
    import { mapGetters, mapMutations, mapActions } from 'vuex'
    export default {
        name: 'editRoomModal',
        data() {
            return {
                formItemLayout: {
                    labelCol: {
                        xs: { span: 12},
                        sm: { span: 6 },
                    },
                    wrapperCol: {
                        xs: { span: 24 },
                        sm: { span: 16 },
                    },
                },
            }
        },
        computed: {
            ...mapGetters([
                'editRoomModalVisible',
                'currentRoom'
            ])
        },
        beforeCreate() {
            this.form = this.$form.createForm(this, { name: 'editRoomModal' });
        },
        mounted() {

        },
        methods: {
            ...mapMutations([
                'set_editRoomModalVisible',
                'set_editRoomParams'
            ]),
            ...mapActions([
                'editRoom'
            ]),
            cancel() {
                this.set_editRoomModalVisible(false)
            },
            handleSubmit(e) {
                e.preventDefault();
                this.form.validateFieldsAndScroll((err, values) => {
                    if (!err) {
                        let obj={}
                        obj.area=this.form.getFieldValue('area')
                        obj.level=this.form.getFieldValue('level')
                        obj.bedType=this.form.getFieldValue('bedType')
                        obj.addBed=this.form.getFieldValue('addBed')
                        obj.facility=this.form.getFieldValue('facility')
                        const data = {
                            roomType: this.form.getFieldValue('roomType'),
                            price: Number(this.form.getFieldValue('price')),
                            total: Number(this.form.getFieldValue('roomNum')),
                            curNum: Number(this.form.getFieldValue('roomNum')),
                            detail:JSON.stringify(obj),
                            id:this.currentRoom.id
                        }
                        this.set_editRoomParams(data)
                        this.editRoom()
                    }
                });
            },
        }
    }
</script>